...
export class FileService {
  ...
  async getFilesByFolder(folderId: string | null): Promise<File[]> {
    // If folderId is null, returns files in root folder
    if (folderId) {
  ...
  async createFile(data: {
    ...
    // If folderId is not provided (null/undefined), file will be created in root
    if (data.folderId) {
    ...
    return this.fileRepository.create({
      ...
      folderId: data.folderId || undefined, // undefined means root folder
    });
  }
